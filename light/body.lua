...
local function new(id, type, ...)
  ...
  obj.is_on_screen = true
    ...
    obj.imgWidth = args[4]
    obj.imgHeight = args[5]
    obj:generateNormalMapFlat("top")
    obj:setShadowType('rectangle', args[4] or obj.imgWidth, args[5] or obj.imgHeight)
		obj.reflective = true
...
function body:setNormalMap2(normal, x, y, width, height)
		if normal then
			self.normal = normal
			--self.normal:setWrap("repeat", "repeat")
			self.normalWidth = width or self.normal:getWidth()
			...
			self.normalVert = {
				{0.0, 0.0, ((0 + x) * self.normalWidth) / self.normal:getWidth(), ((0 + y) * self.normalHeight) / self.normal:getHeight()},
				{self.normalWidth, 0.0, ((1 + x) * self.normalWidth) / self.normal:getWidth(), ((0 + y) * self.normalHeight) / self.normal:getHeight()},
				{self.normalWidth, self.normalHeight, ((1 + x) * self.normalWidth) / self.normal:getWidth(), ((1 + y) * self.normalHeight) / self.normal:getHeight()},
				{0.0, self.normalHeight, ((0 + x) * self.normalWidth) / self.normal:getWidth(), ((1 + y) * self.normalHeight) / self.normal:getHeight()}
			}
			self.normalMesh = love.graphics.newMesh(self.normalVert, self.normal, "fan")
		else
			self.normalMesh = nil
